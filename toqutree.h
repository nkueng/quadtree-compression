...
public:
   ...
   * This function will call helper function buildTree.
    ...
    * @return image of current node
    */
   PNG renderNode(Node * node);

    /**
    * Combines 4 sub-images (k) into a single one of dimension k+1
    * @param XX_im, pointers to sub-images
    * @param k, dimension of node and image to be returned
    * @param ctr, split point of current node used as reference for overlaying
    * @return combined image of 4 sub-images
    */
   PNG combine4Ims(PNG * NW_im, PNG * NE_im, PNG * SW_im, PNG * SE_im, int k, pair<int, int> ctr);

    /**
    * Copies content of sub-image into image
    * @param im, image of dimension k
    * @param sub_im, sub-image of dimension k-1
    * @param ul, coordinates of upper-left corner of sub_im in im
    */
   void overlayIm(PNG * im, PNG * const sub_im, pair<int, int> ul);
   ...
   /*
    *  Recursive helper function for pruning.
    *  @param node, pointer to node candidate checked for pruning
    *  @param tol, tolerance value average hues of children must be in to be pruned
    */
   void prune(Node * node, double tol);

   /*
    *  Recursive function to check tolerance of all children of root at node
    *  @param node, pointer to node candidate checked for pruning
    *  @param avg, average hue of currently evaluated node
    *  @param tol, tolerance value average hues of children must be in to be pruned
    *  @return true only if all children are within the requested tolerance
    */
    ...
    *  @param node, pointer to node candidate checked for pruning
    *  @return number of nodes in toqutree
    */
...
private:
   ...
   * @return returns a pointer to the root of the tree that represents the input PNG. 
   */
   Node * buildTree(PNG * im, int k);

   /**
   * @param PNG * im, pointer to a 2^d by 2^d image
   * @param int k, dimension of **search region** for split point, k = d - 1
   * @param stats * curr_stats_ptr, pointer to dynamic stats object for im
   * @return, the coordinate pairs corresponding to the relevant split coordinates
   */ 
   vector< pair<int, int> > findSplit(PNG * im, int k, stats * curr_stats_ptr);

  /**
   * @param PNG * im, pointer to a 2^d by 2^d image
   * @param int k, dimension of output image, k = d - 1
   * @param ul, upper-left coordinate of output image in input image im
   * @param lr, lower-right coordinate of output image in input image im
   * @return, ptr to dynamically allocated image of dim k
   */ 
   PNG * splitIm(PNG * im, int k, pair<int, int> ul, pair<int, int> lr);
